...
// ── Out-pitch sequences + finish location ────────────────────────────────────
// A finish location is only surfaced with >= MIN_OUTPITCH_FINISHES located finishes
// on the out pitch, and only when the modal zone is >= OUTPITCH_MODAL_SHARE of them.
const MIN_OUTPITCH_FINISHES = 5;
const OUTPITCH_MODAL_SHARE = 0.40;

function finishingToken(shortSequence) {
  return shortSequence.split(' → ').pop();
}

/**
 * Among the out sequences whose finishing pitch is `outPitchType`, finds the modal
 * finish zone. Only located finishes (zone != null) count toward both the pool and
 * the denominator. Returns { zone, count, total } when the pool has at least
 * MIN_OUTPITCH_FINISHES finishes and the modal zone is at least OUTPITCH_MODAL_SHARE
 * of them (inclusive); otherwise null. A modal tie is broken by first appearance.
 * @param {Array<Object>} outSequences - out records ({ shortSequence, zone, ... }).
 * @param {string|null} outPitchType - the out pitch (finishing token) to pool on.
 * @returns {{zone:string,count:number,total:number}|null}
 */
function computeOutPitchLocation(outSequences, outPitchType) {
  if (!outPitchType) return null;
  const pool = outSequences.filter(o => finishingToken(o.shortSequence) === outPitchType && o.zone != null);
  if (pool.length < MIN_OUTPITCH_FINISHES) return null;
  const counts = new Map(); // insertion order preserved → first-seen wins ties
  for (const o of pool) counts.set(o.zone, (counts.get(o.zone) || 0) + 1);
  let modalZone = null, modalCount = 0;
  for (const [zone, count] of counts) {
    if (count > modalCount) { modalZone = zone; modalCount = count; }
  }
  if (modalCount / pool.length < OUTPITCH_MODAL_SHARE) return null;
  return { zone: modalZone, count: modalCount, total: pool.length };
}

/**
 * Summarizes the pitch sequences that get a batter out. Returns { text, breakdown,
 * location }; `text` and `breakdown` are byte-for-byte identical to the prior inline
 * implementation. `location` adds the modal finish zone of the out pitch (see
 * computeOutPitchLocation), or null. Extracted from server.js so it can be unit-tested.
 * @param {Array<Object>} outSequences - out records ({ shortSequence, outType, wasSwinging, zone }).
 * @returns {{text:string, breakdown:Object|null, location:Object|null}}
 */
function analyzeOutSequences(outSequences) {
  if (!outSequences || outSequences.length === 0) {
    return { text: 'Insufficient data', breakdown: null, location: null };
  }
  ...
  if (significantSequences.length > 0) {
    ...
    const outPitchType = finishingToken(topSeq);
    return {
      text,
      breakdown: buildBreakdown(topSeq, matchingOuts),
      location: computeOutPitchLocation(outSequences, outPitchType),
    };
  }
  ...
  if (!topPitch) return { text: 'Insufficient data', breakdown: null, location: null };
  ...
  return {
    ...
    location: computeOutPitchLocation(outSequences, topPitch[0]),
  };
}
...
module.exports = {
  ...
  MIN_OUTPITCH_FINISHES,
  OUTPITCH_MODAL_SHARE,
  finishingToken,
  computeOutPitchLocation,
  analyzeOutSequences,
};
